...
namespace StackExchange.Profiling
{
    ...
    public partial class MiniProfiler
    {
        ...
        {
            get {
                if (_heads == null) _heads = new Dictionary<int, Timing>();
                if (_heads.ContainsKey(Thread.CurrentThread.ManagedThreadId))
                    return _heads[Thread.CurrentThread.ManagedThreadId];
                else
                    return null;
            }
            set
            {
                if (_heads == null) _heads = new Dictionary<int, Timing>();
                if(value != null)
                    _heads[Thread.CurrentThread.ManagedThreadId] = value;
            }
        }

        private Dictionary<int, Timing> _heads { get; set; }

        public void SetHeadForChildThread(int parentThreadId, int childThreadId) 
        {
            if (_heads == null) return;

            if (_heads.ContainsKey(parentThreadId))
                _heads[childThreadId] = _heads[parentThreadId];
        }
        ...
        public MiniProfiler(string url, ProfileLevel level = ProfileLevel.Info)
        {
            Id = Guid.NewGuid();
            Level = level;
            _heads = new Dictionary<int, Timing>();
